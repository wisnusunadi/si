    ...
    function getDraftPerakitan(Request $request)
    {
        if ($request->id) {
            ...
            return datatables()->of($data)
                ...
                ->addColumn('action', function ($d) {
                    return '<a data-toggle="modal" data-target="#detail" class="detail" data-attr="" data-gbj='.$d->gdg_brg_jadi_id.' data-id="' . $d->id . '">
                                <button class="btn btn-info"><i
                                class="far fa-eye"></i> Detail</button>
                            </a>';
                })
    ...
    function storeDraftRancang(Request $request)
    {
        ...
        foreach ($request->gdg_brg_jadi_id as $key => $value) {
            ...
            foreach($request->noseri[$value] as $k => $v) {
                ...
                $n->noseri_id = $nn->id;
                $n->layout_id = json_decode($request->layout[$value][$k], true);
                $n->jenis = 'keluar';
                $n->status_id = 1;
                $n->state_id = 2;
                $n->save();
