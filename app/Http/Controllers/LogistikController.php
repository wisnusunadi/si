...
class LogistikController extends Controller
{
    ...
    public function get_data_detail_belum_kirim_so($id, $jenis)
    {
        ...
        if ($jenis == "EKAT") {
            $datas = DetailPesananProduk::addSelect([
            ...
            ])->WhereHas('NoSeriDetailPesanan', function ($q) {
                $q->whereIN('status', ['ok']);
            ...
            foreach ($datas as $i) {
                $id = $i->id;
                $jumlahterkirim = NoseriDetailLogistik::leftJoin('noseri_detail_pesanan', 'noseri_detail_pesanan.id', '=', 'noseri_logistik.noseri_detail_pesanan_id')
                    ->leftJoin('riwayat_batal_po_seri', 'riwayat_batal_po_seri.t_tfbj_noseri_id', '=', 'noseri_detail_pesanan.t_tfbj_noseri_id')
                    ->whereHas('DetailLogistik', function ($q) use ($id) {
                        $q->where('detail_pesanan_produk_id', $id);
                    })
                    ->whereNull('riwayat_batal_po_seri.id')
                    ->count();
                $jumlahsudahuji = NoseriDetailPesanan::leftJoin('riwayat_batal_po_seri', 'riwayat_batal_po_seri.t_tfbj_noseri_id', '=', 'noseri_detail_pesanan.t_tfbj_noseri_id')
            ...
            return datatables()->of($data)
                ...
                ->addColumn('array_check', function ($data) {
                    if (isset($data->gudangbarangjadi)) {
                        ...
                        $s = NoseriDetailPesanan::select('noseri_detail_pesanan.id')
                            ->leftJoin('riwayat_batal_po_seri', 'riwayat_batal_po_seri.t_tfbj_noseri_id', '=', 'noseri_detail_pesanan.t_tfbj_noseri_id')
                            ->where(['noseri_detail_pesanan.status' => 'ok', 'noseri_detail_pesanan.detail_pesanan_produk_id' => $id, 'noseri_detail_pesanan.is_ready' => 0])
                            ->whereNull('riwayat_batal_po_seri.id')
            ...
            $datapart = DetailPesananPart::where('pesanan_id', $pesanan_id)->DoesntHave('RiwayatBatalPoPart')->get();

            $datas_jasa = DetailPesananPart::where('pesanan_id', $pesanan_id)->whereHas('Sparepart', function ($q) {
                $q->where('kode', 'LIKE', '%JASA%');
            })->DoesntHave('DetailLogistikPart')->DoesntHave('RiwayatBatalPoPart')->get();
            ...
            return datatables()->of($data)
                ...
                ->addColumn('jumlah', function ($data) {
                    if (isset($data->gudangbarangjadi)) {
                        ...
                        $jumlahsudahuji = NoseriDetailPesanan::leftJoin('riwayat_batal_po_seri', 'riwayat_batal_po_seri.t_tfbj_noseri_id', '=', 'noseri_detail_pesanan.t_tfbj_noseri_id')
                            ->where(['noseri_detail_pesanan.status' => 'ok', 'detail_pesanan_produk_id' => $id, 'is_ready' => 0])
                            ->whereNull('riwayat_batal_po_seri.id')
                            ->count();
                ...
                ->addColumn('array_check', function ($data) {
                    if (isset($data->gudangbarangjadi)) {
                        ...
                        $s = NoseriDetailPesanan::select('noseri_detail_pesanan.id')
                            ->leftJoin('riwayat_batal_po_seri', 'riwayat_batal_po_seri.t_tfbj_noseri_id', '=', 'noseri_detail_pesanan.t_tfbj_noseri_id')
                            ->where(['noseri_detail_pesanan.status' => 'ok', 'is_ready' => 0, 'detail_pesanan_produk_id' => $id])
                            ->whereNull('riwayat_batal_po_seri.id')
                            ->DoesntHave('NoseriDetailLogistik')->get();
                        return '<div name="array_check[]">' . $s->implode('id', ',') . '</div>';
                    }
                })
                ->addColumn('button', function ($data) {
    ...
    public function get_data_so($value, $years)
    {
        ...
        if ($value == "belum_kirim") {
            $data = Pesanan::addSelect([
                ...
                'cbatalpo' => function ($q) {
                    $q->selectRaw('count(riwayat_batal_po_seri.id)')
            ...
            $data = Pesanan::addSelect([
                ...
                ->whereNotNull('no_po')
                ->whereYear('created_at',  $years)
                ->havingRaw('(((cqcprd > 0 AND clogprd < cqcprd) OR clogprd = 0 OR ((cpoprd - cbatalpo) > clogprd ) ) AND cpoprd > 0 ) OR (((cqcpart > 0 AND clogpart < cqcpart) OR clogpart = 0 ) AND cpopart > 0 ) OR  (((ctfjasa > 0 AND clogjasa < ctfjasa )OR clogjasa = 0 ) AND ctfjasa > 0 )')
                ->orderBydesc('created_at')
                ->get();
        }
        ...
        function getPersentase($data)
        {
            ...
            $tes = $data->clogprd + $data->clogpart + $data->clogjasa;
            if ($res > 0) {
                $hitung = floor(((($data->clogprd + $data->clogpart + $data->clogjasa) / (($data->cpoprd - $data->cbatalpo) + $data->cqcpart + $data->ctfjasa)) * 100));
                if ($hitung > 0) {
                    $datas = $hitung;
                } else {
                    $datas = $hitung;
    ...
    public function get_data_selesai_so($years)
    {
        $data = Pesanan::addSelect([
            ...
            ->whereNotNull('no_po')
            ->whereYear('created_at',  $years)
            ->havingRaw('(((cpoprd - cbatalpo) = clogprd AND clogprd > 0 ) AND cpoprd > 0 ) OR ((clogpart = cqcpart AND clogpart > 0 )  AND cpopart > 0 ) OR  (((clogjasa = ctfjasa ) AND  clogjasa > 0 ) AND ctfjasa > 0 )')
            // ->havingRaw('(((cqcprd > 0 AND clogprd < cqcprd) OR clogprd = 0 OR ((cpoprd - cbatalpo) > clogprd ) ) AND cpoprd > 0 ) OR (((cqcpart > 0 AND clogpart < cqcpart) OR clogpart = 0 ) AND cpopart > 0 ) OR  (((ctfjasa > 0 AND clogjasa < ctfjasa )OR clogjasa = 0 ) AND ctfjasa > 0 )')
            ->orderBydesc('created_at')
            ->get();
