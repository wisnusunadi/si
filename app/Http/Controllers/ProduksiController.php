...
use App\Models\JadwalRakitNoseriNonStok;
...
class ProduksiController extends Controller
{
    ...
    function detail_non_jadwal($id)
    {
        $head = JadwalPerakitan::select(
            ...
            ->where('jadwal_perakitan.id', $id)
            ->first();



        $data = JadwalRakitNoseri::select('jadwal_rakit_noseri.noseri', 'jadwal_rakit_noseri.id')
            ->where('jadwal_rakit_noseri.jadwal_id', $id)
            ->get();
        ...
        if (!$head) {
            ...
            $obj = (object)[
                'id' => $head->id,
                'no_bppb' => $head->no_bppb,
                'nama' => $head->nama . ' ' . $head->variasi,
                'jumlah' => $head->jumlah,
                'tgl_buat' => $head->created_at,
                'divisi' => $head->divisi,
                'seri' => $seri,
                'tujuan' => $head->keterangan
            ];
        }
        return response()->json($obj);
    }
        ...
        if ($data->isEmpty()) {
            ...
            foreach ($data as $d) {
                $obj[] = array(
                    'id' => $d->id,
                    'no_bppb' => $d->no_bppb,
                    'produk' => $d->nama . ' ' . $d->variasi,
                    'jml' => $d->jumlah,
                    'tgl_rakit' => $d->created_at,
                    'bagian' => $d->divisi,
                );
            }
        }
        ...
        try {
            ...
            if ($prd->kode != NULL || $prd->kode != '') {
                $jadwal = JadwalPerakitan::create([
                    ...
                    'no_bppb' => $obj->no_bppb,
                    'produk_id' => $gbj->id,
                    'jumlah' => $obj->produk->isGenerate ? $obj->jml : count($obj->noseri),
                ...
                if ($obj->produk->isGenerate) {
                    ...
                    if (in_array($gbj->kode, ['AA', 'AB'])) {
                        for ($i = 1; $i <= $obj->jml; $i++) {
                            $newSeri[] = $gbj->kode . str_pad($bulan, 2, '0', STR_PAD_LEFT) . $tahun . str_pad($max + $i, 7, '0', STR_PAD_LEFT);
                            $newSeries[] = array(
                                'jadwal_id' => $jadwal->id,
                                'no_bppb' => $obj->no_bppb,
                                'no_urut' => $max + $i,
    ...
    function getOutSO()
    {
        try {
            ...
            $datas = Pesanan::addSelect([
                ...
                ->havingRaw('cjumlahprd - count_batal > 0 AND cjumlahprd - count_batal != count_gudang_tidak_batal ')
            ...
            foreach ($datas as $d) {
                ...
                $obj[] = array(
                    ...
                    'tgl_po' => $d->tgl_po,
                    'tgl_kontrak' => $batas,
                    'jumlah_gdg' => $d->count_gudang_tidak_batal,
                    'jumlah_siap' => $d->csiap,
                    'jumlah' => $d->cjumlahprd - $d->count_batal,
                ...
                foreach ($paket as $key_a => $p) {
                    ...
                    foreach ($p->DetailPesananProdukVariasi() as $key_b => $i) {
                        $obj[$key_a]['item'][$key_b] = array(
                            ...
                            'jumlah_sisa' => $i->count_jumlah  - $i->count_batal - $i->count_gudang_tidak_batal,
                            /// 'status' => $i->status_cek == NULL || $i->checked_by == NULL ||  $sisa == $i->count_batal ? false : true,
                            'status' => $i->status_cek == NULL || $i->checked_by == NULL || $i->count_jumlah  - $i->count_batal ==  $i->count_gudang_tidak_batal ? false : true,
                        );
                        // $obj[$key_a]['item'][$key_b] = array(
                        //     'id' => $i->id,
                        //     'gudang_id' => $i->gudang_barang_jadi_id,
    ...
    function on_rakit_detail($id)
    {
        $date = Carbon::now();
        $tahun = $date->format('Y') % 100;
        $produk = GudangBarangJadi::select('produk.kode', 'gdg_barang_jadi.id as gbj_id')
            ->leftJoin('produk', 'produk.id', '=', 'gdg_barang_jadi.produk_id')
            ->where('gdg_barang_jadi.id', $id)
            ->first();

        $data = JadwalRakitNoseri::where('jadwal_rakit_noseri.th', $tahun)
    ...
    function on_rakit()
    {
        try {
            ...
            $data = JadwalPerakitan::select(
                ...
                'p.id as produk_id',
                'jadwal_perakitan.created_at',
                'jadwal_perakitan.tanggal_mulai',
                'jadwal_perakitan.tanggal_selesai',
                'jadwal_perakitan.no_bppb',
    ...
    function jml_sisa_transfer($d)
    {
        ...
        return [
            'sisa_kirim' => intval($seri_belum),
            'sisa_rakit' => intval($d->jumlah - $c_all)
        ];
    }
    ...
    {
        try {
            $data = JadwalPerakitan::whereNotNull('keterangan_transfer')->orWhereNotNull('keterangan')->get()->map(function ($item) {
                return [
                    ...$item->toArray(),
                    'produk' => $item->produk->nama ? $item->produk->produk->nama . ' ' . $item->produk->nama : $item->produk->produk->nama,
                    'jml_rakit' => $item->jumlah . ' ' . $item->produk->satuan->nama,
                    'jml_sisa' => $this->jml_sisa_transfer($item),
                    'remark' => $item->keterangan ? $item->keterangan : $item->keterangan_transfer,
                ];
            });
