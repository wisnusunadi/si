    ...
    data() {
        return {
            ...
            rules: {
                ...
                kodeUnique: (id, value) => {
                    return id ? true :
                        !this.product.some(item => item.kode === value) || 'Kode produk sudah ada'
                }
            },
            loading: false,
        }
                ...
                <v-form ref="formProducts" v-model="valid" lazy-validation>
                    <v-data-table :headers="header" :items="selectProduct">
                        ...
                        <template #item.kode="{ item }">
                            <v-text-field class="mt-5" v-model="item.kode" @input="item.kode = item.kode.toUpperCase()"
                                :rules="[rules.kodeUnique(item.id, item.kode)]" outlined dense></v-text-field>
                        </template>

                        <template #item.nama="{ item }">
                            <v-text-field class="mt-5" v-model="item.nama" @input="item.nama = item.nama.toUpperCase()"
                        ...
                        <template #item.no_akd="{ item }">
                            <v-text-field type="number" class="mt-5" v-model="item.no_akd" :rules="[rules.mustBeNumber]"
                        ...
                        <template #item.generate_seri="{ item }">
                            <v-switch class="mt-5 mx-1" v-model="item.generate_seri" color="primary"></v-switch>
                        </template>
