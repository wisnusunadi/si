        ...
        <v-dialog v-model="dialogCreate" persistent max-width="70%">
            <v-card>
                ...
                <v-form ref="formCategory" v-model="valid" lazy-validation>
                    <v-data-table :headers="header" :items="selectCategory">
                        ...
                        <template #item.kode="{ item }">
                            <v-text-field class="mt-5" v-model="item.kode" label="Kode Kategori" outlined
                                dense></v-text-field>
                        </template>

                        <template #item.nama="{ item }">
                            <v-text-field class="mt-5" v-model="item.nama" label="Nama Kategori" outlined dense
                                @input="item.nama = item.nama.toUpperCase()"
                                :rules="[rules.required, rules.unique(item.id, item.nama)]"></v-text-field>
                        </template>

                        <template #item.action="{ item, index }">
                            <v-btn v-if="!item.id" color="red" text @click="removeKategori(item)">
