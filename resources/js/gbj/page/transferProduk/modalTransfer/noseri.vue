...
export default {
  ...
  methods: {
    async getData() {
      try {
        ...
        if (this.detailSelected?.noseri) {
          this.noSeriSelected = JSON.parse(
            JSON.stringify(this.detailSelected.noseri)
          );
        }
      } catch (error) {
        console.error(error);
    ...
    noseriterpakai(item) {
      ...
      for (let i = 0; i < this.allPaket.length; i++) {
        for (let j = 0; j < this.allPaket[i].item.length; j++) {
          ...
          if (
            this.allPaket[i].item[j].noseri.find(
              (noseri) => noseri.id === item.id
            )
          ) {
    ...
    checkAllData() {
      ...
      if (this.checkAll) {
        this.noSeriSelected = this.noseri.filter(
          (noseri) => !this.noseriterpakai(noseri)
        );
      } else {
        this.noSeriSelected = [];
      }
    ...
    simpanSeri() {
      ...
      paket.item.find((produk) => produk.id === this.detailSelected.id).noseri =
        this.noSeriSelected;
...
<template>
  <div>
    <seriviatext
      v-if="showmodalviatext"
      @close="closeModalViaText"
      @submit="submit"
    />
    <div
        ...
        <div class="modal-content">
          ...
          <div class="modal-body">
            <small>
              ...
              Nomor seri yang dipilih tidak boleh lebih dari
              {{ detailSelected.jumlah_sisa }}
            </small>
            <div class="d-flex bd-highlight">
              <div class="p-2 flex-grow-1 bd-highlight">
                <button class="btn btn-primary" @click="clickModalViaText">
              ...
              <template #item.id="{ item }">
                <div v-if="!noseriterpakai(item)">
                  <input
                    ...
                    :checked="
                      noSeriSelected &&
                      noSeriSelected.find((noseri) => noseri.id === item.id)
                    "
                  />
                </div>
                <div v-else>
