...
const ProdukComponent = ({ formProduk, setFormProduk }) => {
    ...
    const handleInputProduk = async (value, index, key) => {
        ...
        if (key == "id_produk") {
            ...
            newProduk[index].variasi = mappingDataVariasiProduk(variasi[0]?.produk)
        ...
        if (item !== undefined) {
            ...
            return item.map((item) => ({
                ...
                gudang_barang_jadi: item.gudang_barang_jadi.map((gudang) => ({
                    value: gudang.id,
                    label: gudang.nama == "" ? item.nama : gudang.nama,
                    stok: gudang.stok,
                })),
            }));
        }
                                        ...
                                        <tr key={index}>
                                            ...
                                            <td>
                                                ...
                                                {item.id_produk !==
                                                    undefined && (
                                                    <div className="pt-4">
                                                        <b className="mb-5">Detail Produk</b>
                                                        {
                                                            item?.variasi?.map((item, index) => (
                                                                <div key={index} className="card-body bg-blue-produk">
                                                                    <h6>{ item.label }</h6>
                                                                </div>
                                                            ))
                                                        }
