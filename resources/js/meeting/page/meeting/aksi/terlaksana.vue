  ...
  methods: {
    ...
    async getDataKaryawan() {
      try {
        ...
        const { data: lokasi } = await this.$_get("/api/hr/meet/lokasi/show");
        ...
        this.meeting.peserta = this.meeting.peserta.filter((peserta) => {
          return (
        ...
        this.selectedParticipants = this.meeting.peserta.map((peserta) => {
        ...
        if (Array.isArray(item)) {
          if (item.length === 0) {
            ...
            return item.some((subItem, index) => {
              ...
              const isSubItemEmpty = Object.entries(subItem).some(
                ([subKey, subItem]) => {
                  if (subItem === "") {
              ...
              if (isSubItemEmpty) {
                emptyFields.push(
                  `${key} baris ${index + 1} ${subEmptyFields.join(", ")}`
                );
              }
        ...
        <div class="modal-body" v-if="!loading">
          ...
          <div class="form-group">
            ...
            <vue-select
              ...
              @input="
                meeting.peserta = selectedParticipants.map((item) => item.id)
              "
            />
          </div>
          <div class="form-group row">
