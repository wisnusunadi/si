  ...
  methods: {
    ...
    async generateSeri() {
      ...
      const noSeriUnique = this.noseri.filter((data, idx) => {
        return (
          this.noseri.findIndex((data2) => data2.seri === data.seri) === idx
        );
      });
      ...
      if (
        !this.isDisable &&
        cek.length === 0 &&
        noSeriUnique.length === this.noseri.length
      ) {
        try {
          this.isDisable = true;
      ...
      if (
        ...
        try {
          ...
          const { data } = await axios.put(
            `/api/prd/rw/gen/${this.selectSeri.id}`,
            {
              noseri: this.noseri,
            }
          );
...
<template>
  <div>
    <modalSeri
      ...
      :hasilGenerate="hasilGenerate"
    />
    <div
      class="modal fade modalSet"
                ...
                <div class="col" v-if="hasilGenerate">
                  <div class="row">
                    <div class="col">
                      ...
                      <generatePackingList
                        :dataTable="noseriGeneratePackingList"
                      />
