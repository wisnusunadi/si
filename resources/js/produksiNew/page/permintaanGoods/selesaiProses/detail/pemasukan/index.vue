...
export default {
    ...
    methods: {
        ...
        statusText(status) {
            switch (status) {
                ...
                case "barang_masuk":
                    return {
                        text: "Barang Masuk",
                        class: "fas fa-truck-loading text-primary",
                    };
        ...
        checkedAll() {
            ...
            if (this.checkAll) {
                this.noSeriSelected = this.itemsNoSeri.filter(
                    (n) => n.status !== "barang_masuk"
                );
            } else {
                this.noSeriSelected = [];
            }
        ...
        filterNoSeri() {
            ...
            if (this.filterSelected.length > 0) {
                filter = filter.filter((item) =>
                    this.filterSelected.includes(item.status)
                );
            }
    ...
    watch: {
        noSeriSelected() {
            if (
                this.noSeriSelected.length ===
                this.itemsNoSeri.filter((n) => n.status !== "barang_masuk")
                    .length
            ) {
                this.checkAll = true;
            } else {
                    ...
                    <div class="d-flex bd-highlight">
                        <div class="p-2 flex-grow-1 bd-highlight">
                            <span class="filter">
                                ...
                                <div
                                    class="dropdown-menu"
                                    aria-labelledby="filterpenjualan"
                                    style=""
                                >
                                    <form
                                        class="px-4"
                        ...
                        <div class="p-2 bd-highlight">
                            <input
                                type="text"
                                class="form-control"
                                v-model="searchNoSeri"
                                placeholder="Cari..."
                            />
                        </div>
                    </div>
                    <data-table
                        :headers="headersNoSeri"
                        :items="filterNoSeri"
                        :search="searchNoSeri"
                    >
                        <template #header.id>
                        ...
                        <template #item.id="{ item }">
                            <input
                                ...
                                v-if="item.status != 'barang_masuk'"
                                :checked="
                                    noSeriSelected &&
                                    noSeriSelected.find((n) => n.id === item.id)
                                        ? true
                                        : false
                                "
                            />
