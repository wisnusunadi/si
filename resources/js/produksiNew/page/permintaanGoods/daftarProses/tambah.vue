...
export default {
    ...
    methods: {
        ...
        simpan() {
            ...
            const form = Object.entries(this.form).every(([key, value]) => {
                if (key === "tgl_pengembalian") {
                    return this.form.jenis.value === "peminjaman"
                        ? value
                        : true;
                }
                return value;
            ...
            const items = this.items.every(
                (item) => item.nama_produk && item.jumlah
            );

            //  jika ada produk yang stok nya 0, tampilkan peringatan
            if (this.items.some((item) => item.stok === 0)) {
                swal.fire("Peringatan", "Produk tidak tersedia", "warning");
                return;
            }
        ...
        checkProdukFilled(idx) {
            const selectedProduk = this.items.map(
                (item) => item.nama_produk?.value
            );

            return this.produkChoices.filter((produk) => {
                return (
                    !selectedProduk.includes(produk.value) ||
                    this.items[idx].nama_produk?.value === produk.value
                );
            });
        },
    },
        ...
        items: {
            handler: function (val) {
                val.forEach((item, index) => {
                    if (item.nama_produk) {
                        const produk = this.produkChoices.find(
                            (p) => p.value === item.nama_produk.value
                        );
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div
                                ...
                                v-if="form?.jenis?.value == 'peminjaman'"
                            >
                                <label class="col-5 text-right"
                                ...
                                <tbody>
                                    ...
                                    >
                                        ...
                                        <td>
                                            <v-select
                                                :options="
                                                    checkProdukFilled(index)
                                                "
                                                v-model="item.nama_produk"
                                            ></v-select>
                                        </td>
                                        <td>
                                            <input
