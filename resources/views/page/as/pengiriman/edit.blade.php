                                    ...
                                    <div class="card card-outline card-warning">
                                        ...
                                        <div class="card-body">
                                            <div class="form-group">
                                                ...
                                                <p class="text-muted mt-1" id="tgl_retur">{{ $i->ReturPenjualan->no_retur }}
                                                </p>
                                            </div>
                                            <div class="form-group">
                                                <strong>Tanggal Retur</strong>
                                                <p class="text-muted mt-1" id="tgl_retur">
                                                    {{ $i->ReturPenjualan->tgl_retur }}</p>
                                            </div>
                                            <div class="form-group">
                                                <strong>Jenis Retur</strong>
                                                <p class="text-muted mt-1" id="jenis_retur">{{ $i->ReturPenjualan->jenis }}
                                                </p>
                                            </div>
                                            <div class="form-group">
                                                <strong>No Referensi Penjualan</strong>
                                                <p class="text-muted mt-1" id="no_pesanan">
                                                    @if ($i->ReturPenjualan->pesanan_id != null)
                                                        {{ $i->ReturPenjualan->Pesanan->no_po }}
                                                    @elseif($i->ReturPenjualan->retur_penjualan_id != null)
                                                        {{ $i->ReturPenjualan->ReturPenjualanChild->no_retur }}
                                                    @else
                                                        {{ $i->ReturPenjualan->no_pesanan }}
                                                    @endif
                                                </p>
                                            </div>
                                            <div class="form-group">
                                        ...
                                        <div class="card-body">
                                            <div class="form-group row">
                                                <label for="nama_penerima"
                                                    class="col-lg-5 col-md-12 col-form-label labelket">Nama Penerima</label>
                                                <div class="col-lg-4 col-md-8">
                                                    <input type="text" name="nama_penerima" id="nama_penerima"
                                                        class="nama_penerima form-control text-sm"
                                                        value="{{ $i->customer_id != null ? $i->Customer->nama : $i->nama_penerima }}"
                                                        @if ($i->customer_id != null) readonly="true" @endif />
                                                    <input type="text" name="customer_id" id="customer_id"
                                                        class="customer_id form-control hide"
                                                        value="@if ($i->customer_id != null) {{ $i->customer_id }} @endif" />
                                                    <div class="form-check form-check-inline mt-1">
                                                        <input class="form-check-input" type="checkbox" id="pilih_penerima"
                                                            value="customer"
                                                            @if ($i->customer_id != null) checked @endif>
                                                        <label class="form-check-label" for="pilih_penerima">Atur Customer
                                                            sebagai Penerima</label>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="form-group row">
                                                ...
                                                <div class="col-lg-5 col-md-8">
                                                    <textarea class="form-control text-sm" name="alamat_penerima" id="alamat_penerima"
                                                        @if ($i->customer_id != null) readonly="true" @endif>{{ $i->customer_id != null ? $i->Customer->alamat : $i->alamat_penerima }}</textarea>
                                                </div>
                                            </div>
                                            <div class="form-group row">
                                                <label for="telp_penerima"
                                                    class="col-lg-5 col-md-12 col-form-label labelket">Telepon
                                                    Penerima</label>
                                                <div class="col-lg-4 col-md-8">
                                                    <input type="text" name="telp_penerima" id="telp_penerima"
                                                        class="telp_penerima text-sm form-control"
                                                        value="{{ $i->customer_id != null ? $i->Customer->telp : $i->telepon_penerima }}"
                                                        @if ($i->customer_id != null) readonly="true" @endif />
                                                </div>
                                            </div>
                                        </div>
                                                            ...
                                                            <tbody>
                                                                @if (isset($i->PengirimanNoseri))
                                                                    @foreach ($i->PengirimanNoseri as $key => $i)
                                                                        <tr>
                                                                            <td>{{ $key + 1 }}</td>
                                                                            <td>{{ $i->NoseriBarangJadi->gudang->produk->nama }}
                                                                                {{ $i->NoseriBarangJadi->gudang->nama }}
                                                                            </td>
                                                                            <td><input
                                                                                    name="no_seri_id[{{ $key }}]"
                                                                                    id="no_seri_id{{ $key }}"
                                                                                    class="form-control no_seri_id d-none"
                                                                                    value="{{ $i->NoseriBarangJadi->id }}" />{{ $i->NoseriBarangJadi->noseri }}
                                                                            </td>
                                                                        </tr>
                                                                    @endforeach
                                                                @endif
                                                            ...
                                                            <tbody>
                                                                @if (isset($i->PengirimanPart))
                                                                    @foreach ($i->PengirimanPart as $key => $i)
                                                                        <tr>
                                                                            <td>{{ $key + 1 }}</td>
                                                                            <td><input name="part_id[{{ $key }}]"
                                                                                    id="part_id{{ $key }}"
                                                                                    class="form-control part_id d-none"
                                                                                    value="{{ $i->Sparepart->id }}" />{{ $i->Sparepart->nama }}
                                                                            </td>
                                                                            <td><input
                                                                                    name="part_jumlah[{{ $key }}]"
                                                                                    id="part_jumlah{{ $key }}"
                                                                                    class="form-control part_jumlah d-none"
                                                                                    value="{{ $i->jumlah }}" />{{ $i->jumlah }}
                                                                            </td>
                                                                        </tr>
                                                                    @endforeach
                                                                @endif
            ...
            function return_part(part) {
                ...
                if (part.length > 0) {
                    for (var i = 0; i < part.length; i++) {
                        data += `<tr>
                        <td>` + (i + 1) + `</td>
                        <td><input name="part_id[` + i + `]" id="part_id` + i +
                            `" class="form-control part_id d-none" value="` + part[i].id + `"/>` + part[i]
                            .nama_part + `</td>
                        <td><input name="part_jumlah[` + i + `]" id="part_jumlah` + i +
                            `" class="form-control part_jumlah d-none" value="` + part[i].jumlah + `"/>` + part[i]
                            .jumlah + `</td>
