                                            ...
                                            <span class="float-left ">
                                                <div class="input-group-prepend">
                                                    <button type="button" class="btn btn-outline-success dropdown-toggle"
                                                        data-toggle="dropdown" disabled id="export">
                                                        <i class="far fa-file-excel"></i> &nbsp;Export
                                                    </button>
                                                    <div class="dropdown-menu">
                                                        <a class="dropdown-item"
                                                            href="{{ route('penjualan.rencana.laporan', ['customer' => '0', 'tahun' => '0']) }}"
                                                            id="lap_semua">Laporan Semua</a>
                                                        <a class="dropdown-item"
                                                            href="{{ route('penjualan.rencana.laporan_detail', ['customer' => '0', 'tahun' => '0']) }}"
                                                            id="lap_detail">Laporan Detail</a>
                                                    </div>
                                                </div>
                                            </span>
    ...
    <script>
        $(function() {
            ...
            var showtable = $('#showtable').DataTable({
                ...
                "drawCallback": function(settings) {
                    ...
                    api.column(groupColumn, {
                        ...
                        if (last !== group) {
                            $(rows).eq(i).before(
                                '<tr class="group" style="background-color:steelblue; color:white;"><td colspan="9"><b>' +
                                group + '</b></td></tr>'
                            );
                            last = group;
                        }
            ...
            $(document).on('click', '.realmodal', function(event) {
                ...
                $.ajax({
                    ...
                    error: function(jqXHR, testStatus, error) {
                        console.log(error);
                        alert("Page " + '/penjualan/rencana/real/show/' + id +
            ...
            $(document).on('click', '#realtable input[name="check_all"]', function() {
                var rows = $('#realtable').DataTable().rows({
                    'search': 'applied'
                }).nodes();
            ...
            $(document).on('submit', '#formsubmit', function(e) {
                ...
                var detail_pesanan_id = $('#realtable').DataTable().$('tr').find(
                    'input[type="checkbox"][name="detail_pesanan_id[]"]').serializeArray();
                var data = [];
            ...
            $('#showtable tbody').on('click', '.produk_id', function() {
                ...
                if (data.jumlah_real <= 0) {
                    $(this).closest('.produk_id').editable({
                        tpl: '<select id="produk"><option value="' + data.penjualan_produk_id +
                            '" selected="selected">' + data.produk + '</option></select>',
                        select2: {
