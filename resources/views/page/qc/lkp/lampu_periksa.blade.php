<style>
    .mdtxt {
        font-size: 14px;
    }

    .nospace {
        width: 1%;
        white-space: nowrap;
    }

    .vera {
        vertical-align: top;
    }

    .veramd {
        vertical-align: middle;
    }

    .wb {
        word-break: break-all;
    }
</style>
<div class="card">
    <div class="card-body">
        <div class="row">
            <div class="col-lg-12">
                <div class="table-responsive">
                    <table class="table mdtxt" style="width:100%;" border=1 style="border-collapse: collapse;">
                        <tbody>
                            <tr>
                                <td style="text-align:center;" class="nospace">
                                    <b>PT SINKO PRIMA ALLOY</b> <br>
                                    LEMBAR KERJA PENGUJIAN<br>
                                    LAMPU PERIKSA
                                </td>
                                <td class="nospace vera">
                                    <table>
                                        <tr>
                                            <td>No Dokumen</td>
                                            <td>:</td>
                                            <td>SPA-LKP/QC-11</td>
                                        </tr>
                                        <tr>
                                            <td>Revisi</td>
                                            <td>:</td>
                                            <td>00</td>
                                        </tr>
                                        <tr>
                                            <td>Tgl.Terbit</td>
                                            <td>:</td>
                                            <td>19 Januari 2021
                                            </td>
                                        </tr>

                                    </table>
                                </td>
                                <td class="vera">Diuji Oleh</td>
                                <td class="vera">Disetujui Oleh</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    </div>
</div>
<div class="card">
    <div class="card-body">
        <div class="row">
            <table border=0 class="table" width="100%">
                <tbody>
                    <tr>
                        <td></td>

                    </tr>
                </tbody>
            </table>
        </div>
    </div>
</div>
<div class="card">
    <div class="card-body">
        <div class="row">
            <div class="table-responsive">
                <table class="table mdtxt" width=100%>
                    <tbody class="vera">
                        <tr>
                            <td rowspan="7" class="nospace">A.</td>
                            <td class="nospace">Data Administrasi</td>
                            <td>:</td>
                            <td>SPA/LKP/MEL1000</td>
                        </tr>
                        <tr>
                            <td class="nospace">Tempat Pengujian</td>
                            <td>:</td>
                            <td>Dummy Lorem Ipsum</td>
                        </tr>
                        <tr>
                            <td class="nospace">Ruangan Pengujian</td>
                            <td>:</td>
                            <td>Lab QC</td>
                        </tr>
                        <tr>
                            <td class="nospace">Tanggal Pengujian</td>
                            <td>:</td>
                            <td></td>
                        </tr>
                        <tr>
                            <td class="nospace">Metode Kerja</td>
                            <td>:</td>
                            <td>Pengujian Lampu Periksa</td>
                        </tr>
                        <tr>
                            <td class="nospace">Data Alat</td>
                            <td>:</td>
                            <td>
                                <table width=100% class="vera">
                                    <tbody>
                                    <tr>
                                        <td class="nospace">Nama</td>
                                        <td class="nospace">:</td>
                                        <td class="wb vera">Examination Lamp</td>
                                        <td class="nospace">Tipe</td>
                                        <td class="nospace">:</td>
                                        <td>MEL-01</td>
                                    </tr>
                                    <tr>
                                        <td class="nospace">Merk</td>
                                        <td class="nospace">:</td>
                                        <td>Mentor</td>
                                        <td class="nospace">No Seri</td>
                                        <td class="nospace">:</td>
                                        <td></td>
                                    </tr>
                                    </tbody>
                                </table>
                            </td>
                        </tr>
                        <tr>
                            <td>Berlaku s/d tgl</td>
                            <td class="nospace">:</td>
                            <td></td>
                        </tr>
                    </tbody>
                </table>
            </div>
        </div>
    </div>
</div>
</div>
<div class="card">
    <div class="card-body">
        <div class="row">
            <table border=0 class="table" width="100%">
                <tbody>
                    <tr>
                        <td></td>
                    </tr>
                </tbody>
            </table>
        </div>
    </div>
</div>
<div class="card">
    <div class="card-body">
        <div class="row mdtxt">
            <div>B. Daftar Alat Ukur</div>
            <table class="table" border="1" style="border-collapse: collapse; text-align:center;" width="80%">
                <thead>
                    <tr>
                        <th>No</th>
                        <th>Nama Alat</th>
                        <th>Merk</th>
                        <th>Type</th>
                        <th>No Seri</th>
                    </tr>
                </thead>
                <tbody class="nospace">
                    <tr>
                        <td>1</td>
                        <td>Lux Meter</td>
                        <td>Efervine</td>
                        <td>SPIC200</td>
                        <td>J7341128</td>
                    </tr>
                    <tr>
                        <td>2</td>
                        <td>Power Meter</td>
                        <td></td>
                        <td>U12011</td>
                        <td>707168</td>
                    </tr>
                    <tr>
                        <td>3</td>
                        <td>Thermohygrometer</td>
                        <td>KT</td>
                        <td>903</td>
                        <td>-</td>
                    </tr>
                </tbody>
            </table>
        </div>
    </div>
</div>
<div class="card">
    <div class="card-body">
        <div class="row">
            <table border=0 class="table" width="100%">
                <tbody>
                    <tr>
                        <td></td>

                    </tr>
                </tbody>
            </table>
        </div>
    </div>
</div>
<div class="col">
    <div class="row">
        <div class="col">
            <div class="card">
                <div class="card-body">
                    <table width="100%" class="mdtxt">
                        <tbody>
                            <tr>
                                <td width="38%" class="vera">
                                    <div>C. Pengukuran Kondisi Lingkungan</div>
                                    <table border="1" style="border-collapse: collapse;text-align:center;" width="100%">
                                        <thead>
                                            <tr>
                                                <th rowspan="2">No</th>
                                                <th rowspan="2">Parameter</th>
                                                <th colspan="2">Terukur</th>
                                            </tr>
                                            <tr>
                                                <th>Awal</th>
                                                <th>Akhir</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <tr>
                                                <td class="nospace">1</td>
                                                <td class="wb">Temprature</td>
                                                <td class="nospace"></td>
                                                <td class="nospace"></td>
                                            </tr>
                                            <tr>
                                                <td class="nospace">2</td>
                                                <td class="wb">Kelembapan</td>
                                                <td class="nospace"></td>
                                                <td class="nospace"></td>
                                            </tr>
                                        </tbody>
                                    </table>
                                </td>
                                <td width="5%"></td>
                                <td width="57%" class="vera">
                                    <div>D. Pemeriksaan Kondisi Fisik dan Fungsi</div>
                                    
                                        <table border="1" style="border-collapse: collapse;text-align:center;" width="100%">
                                            <thead>
                                                <tr>
                                                    <th rowspan="2">No</th>
                                                    <th rowspan="2">Bagian Alat</th>
                                                    <th colspan="2">Hasil Pemeriksaan</th>
                                                    <th rowspan="2">Keterangan</th>
                                                </tr>
                                                <tr>
                                                    <th>Fisik</th>
                                                    <th>Fungsi</th>
                                                </tr>
                                            </thead>
                                            <tbody>
                                                <tr>
                                                    <td>1</td>
                                                    <td>Badan/Permukaan</td>
                                                    <td>v</td>
                                                    <td>v</td>
                                                    <td>OK</td>
                                                </tr>
                                                <tr>
                                                    <td>2</td>
                                                    <td>Dimmer</td>
                                                    <td>v</td>
                                                    <td>v</td>
                                                    <td>OK</td>
                                                </tr>
                                            </tbody>
                                        </table>
                                </td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            </div>
        </div>
    </div>
</div>
<div class="card">
    <div class="card-body">
        <div class="row">
            <table border=0 class="table" width="100%">
                <tbody>
                    <tr>
                        <td></td>

                    </tr>
                </tbody>
            </table>
        </div>
    </div>
</div>
<div class="card">
    <div class="card-body">
        <div>E.Pengukuran Lux</div>
                        <table border="1" style="border-collapse: collapse;text-align:center;" width="100%">
                            <thead>
                                <tr>
                                    <th rowspan="2" class="nospace">No</th>
                                    <th rowspan="2" class="wb">Setting</th>
                                    <th colspan="3">Hasil Pengukuran (Lx)</th>
                                    <th rowspan="2">Rata Rata</th>
                                </tr>
                                <tr>
                                    <th>I</th>
                                    <th>II</th>
                                    <th>III</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr>
                                    <td>1</td>
                                    <td class="nospace">Intensitas Cahaya (Lux)</td>
                                    <td class="nospace">5</td>
                                    <td class="nospace">5</td>
                                    <td class="nospace">5</td>
                                    <td class="nospace">5</td>
                                </tr>
                            </tbody>
                        </table>

    </div>
</div>
<div class="card">
    <div class="card-body">
        <div class="row">
            <table border=0 class="table" width="100%">
                <tbody>
                    <tr>
                        <td></td>

                    </tr>
                </tbody>
            </table>
        </div>
    </div>
</div>
<div class="card">
    <div class="card-body">
        <div>F.Pengukuran menggunakan Power Meter</div>
                        <table border="1" style="border-collapse: collapse;text-align:center;" width="100%">
                            <thead>
                                <tr>
                                    <th rowspan="2" class="nospace">No</th>
                                    <th rowspan="2" class="wb">Setting</th>
                                    <th colspan="3">Hasil Pengukuran (Lx)</th>
                                    <th rowspan="2">Rata Rata</th>
                                </tr>
                                <tr>
                                    <th>I</th>
                                    <th>II</th>
                                    <th>III</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr>
                                    <td>1</td>
                                    <td class="nospace">220 Vac</td>
                                    <td class="nospace">5</td>
                                    <td class="nospace">5</td>
                                    <td class="nospace">5</td>
                                    <td class="nospace">5</td>
                                </tr>
                                <tr>
                                    <td>2</td>
                                    <td class="nospace">170 Vac</td>
                                    <td class="nospace">5</td>
                                    <td class="nospace">5</td>
                                    <td class="nospace">5</td>
                                    <td class="nospace">5</td>
                                </tr>
                                <tr>
                                    <td>3</td>
                                    <td class="wb">110 Vac</td>
                                    <td class="nospace">5</td>
                                    <td class="nospace">5</td>
                                    <td class="nospace">5</td>
                                    <td class="nospace">5</td>
                                </tr>
                            </tbody>
                        </table>

    </div>
</div>
<div class="card">
    <div class="card-body">
        <div class="row">
            <table border=0 class="table" width="100%">
                <tbody>
                    <tr>
                        <td></td>

                    </tr>
                </tbody>
            </table>
        </div>
    </div>
</div>
<div class="card">
    <div class="card-body">
        <div class="row">
            <table border=0 class="table mdtxt" width="100%">
                <tbody>
                    <tr>
                        <td width="47%">
                            <div>F. Telaah Teknis Pengujian</div>
                            <table width="100%" border="1" style="border-collapse: collapse;">
                                <thead>
                                    <tr>
                                        <th class="nospace">No</th>
                                        <th>Pengukuran</th>
                                        <th class="nospace" style="text-align:center;">Kategori</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr>
                                        <td>1</td>
                                        <td class="wb">Kinerja</td>
                                        <td class="nospace" style="text-align:center;"><b>OK</b></td>
                                    </tr>
                                </tbody>
                            </table>
                        </td>
                        <td width="5%">
                            <table width="100%">
                                <tbody>
                                    <tr>
                                        <td></td>
                                    </tr>
                                </tbody>
                            </table>
                        </td>
                        <td width="47%" class="vera">
                            <div>G. Kesimpulan Telaah Teknis Pengujian</div>
                            <table width="100%" border="1" style="border-collapse: collapse;">
                                <tbody>
                                    <tr>
                                        <td class="wb">Berdasarkan hasil Pengujian alat kesehatan tersebut dinyatakan:</td>
                                        <td class="nospace" style="font-size:20px;" style="text-align:center;"><b>TIDAK OK</b></td>
                                    </tr>
                                </tbody>
                            </table>
                        </td>
                    </tr>
                </tbody>
            </table>
        </div>
    </div>
</div>
<div class="card">
    <div class="card-body">
        <div class="row">
            <table border=0 class="table" width="100%">
                <tbody>
                    <tr>
                        <td></td>

                    </tr>
                </tbody>
            </table>
        </div>
    </div>
</div>
<div class="card">
    <div class="card-body">
        <table width="100%" class="mdtxt">
            <tbody>
                <tr>
                    <td>
                        <div>H. Saran</div>
                        <table border="1" style="border-collapse: collapse;" width="100%">
                            <tbody>
                                <tr>
                                    <td class="wb">Pada hari minggu ku turut ayah ke kota naik delman istimewa ku duduk dimuka ku duduk samping pak kusir yang sedang bekerja mengendarai kuda supaya baik jalannya HEYYYY!!!!!!</td>
                                </tr>
                            </tbody>
                        </table>
                    </td>
                </tr>
            </tbody>
        </table>
    </div>
</div>
<div class="card">
    <div class="card-body mdtxt">
        <table>
            <tbody>
                <tr>
                    <td style="text-align:center;">Dilarang keras mengutip/memperbanyak dan/atau mempublikasikan seluruh atau sebagain isi dari lembaran kerja  ini tanpa ijin tertulis dari PT.SINKO PRIMA ALLOY.Sertifikat ini sah bila dibubuhi cap PT.SINKO PRIMA ALLOY dan ditandatangani oleh pejabat yang berwenang.</td>
                </tr>
            </tbody>
        </table>
    </div>
</div>